...
// Pad Hours and Minutes with a zero
function padZero(i) {
  if (i < 10) {
    i = "0" + i;
  }
...
// Create a string representation of Hours and Minutes from a Unix epoch value
function formatHour(x) {
  time = new Date(x);
  return padZero(time.getUTCHours()) + ":" + padZero(time.getUTCMinutes());
}

// Convert a formatted Hour string to a Unix epoch date
function epoch(str) {
  time = str.split(":");
  hours = Number(time[0]) * 60 * 60 * 1000;
  minutes = Number(time[1]) * 60 * 1000;
  epo = hours + minutes;
  return epo;
}
// Create a new date from a string and return a timestamp to handle time display
function timestamp(str) {
  e = new Date(str).getTime();
  return Number(e);
}

// Create major ticks every hour and minor ticks every 30 minutes
function filterPips(value, type) {
  if (type === 0) {
    return -1;
  }
  switch (value % (60 * 60 * 1000)) {
    case 0:
      return 1;
    case 30 * 60 * 1000:
      return 2;
...
noUiSlider.create(slider1, {
  ...
  start: [
    ...
    timestamp("1970-01-01T11:00:00Z"),
    timestamp("1970-01-01T12:00:00Z"),
    timestamp("1970-01-01T16:00:00Z"),
  ],
...
//  Adding slider1's ranges to a connects array and giving each range an id
var connectsList = slider1.querySelectorAll(".noUi-connect");
var connects = Array.from(connectsList)
var rngIDs = ["s1-color1", "s1-color2", "s2-color1"];  // used for other ranges, makes it easier to track ids
...
noUiSlider.create(slider2, {
  // Indicate the handle starting positions here
  start: [timestamp("1970-01-01T18:00:00Z"), timestamp("1970-01-01T21:00:00Z")],
...
// Adding slider2's range to our connects array and giving it an id
connects.push(slider2.querySelectorAll(".noUi-connect")[0]);
...
slider1.noUiSlider.on("change", function () {
  max = Number(slider2.noUiSlider.get()[0]) - slider1.noUiSlider.options.step;
  current = Number(slider1.noUiSlider.get()[3]);
  if (current > max) {
    slider1.noUiSlider.setHandle(3, max);
  }
});

slider2.noUiSlider.on("change", function () {
  min = Number(slider1.noUiSlider.get()[3]) + slider2.noUiSlider.options.step;
  current = Number(slider2.noUiSlider.get()[0]);
  if (current < min) {
    slider2.noUiSlider.setHandle(0, min);
  }
});
...
slider1.noUiSlider.on("change", function (values, handle) {
  if (handle < 3 && values[handle + 1] == values[handle]) {
    slider1.noUiSlider.setHandle(
      handle,
      Number(values[handle]) - slider1.noUiSlider.options.step
    );
  }
...
slider2.noUiSlider.on("change", function (values, handle) {
  if (handle < 1 && values[handle + 1] == values[handle]) {
    slider2.noUiSlider.setHandle(
      handle,
      Number(values[handle]) - slider2.noUiSlider.options.step
    );
  }
  if (handle != 0 && values[handle - 1] == values[handle]) {
    slider2.noUiSlider.setHandle(
